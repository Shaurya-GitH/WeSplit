...
public interface ExpenseSplitRepository extends JpaRepository<ExpenseSplit,Long> {
    @Query("select distinct e1.expense from ExpenseSplit e1 where e1.expense.settled=:settled and e1.user=:user1 and e1.expense  in (select e2.expense from ExpenseSplit e2 where e2.user=:user2)")
